...
int ans(int i, vector<int> &c, vector<vector<int>> &adj, vector<int> &memo)
{
    if (memo[i] != -1)
        return memo[i];
    int res = 0;
    for (int j : adj[i])
        res += ans(j, c, adj, memo);
    return memo[i] = min(res, c[i]);
}
